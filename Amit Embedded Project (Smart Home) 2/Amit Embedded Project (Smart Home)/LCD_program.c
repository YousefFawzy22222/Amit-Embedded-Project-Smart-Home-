...
uint8 Cursor_Postion = 0;

void LCD_voidSendCommand(uint8 Copy_u8Command)
{
    DIO_VidSetPinValue(LCD_CTRL_PORT_RS, LCD_RS_PIN, PinLow); // RS read command
    DIO_VidSetPinValue(LCD_CTRL_PORT_RW, LCD_RW_PIN, PinLow); // RW write to LCD

    DIO_VidSetPortValue(LCD_DATA_PORT, Copy_u8Command); // put the data into the port

    // 2ms Pulse
    DIO_VidSetPinValue(LCD_CTRL_PORT_E, LCD_E_PIN, PinHigh); // Enable on
    _delay_ms(2);                                          // wait 2ms
    DIO_VidSetPinValue(LCD_CTRL_PORT_E, LCD_E_PIN, PinLow);  // Enable off
}
...
void LCD_voidSendData(uint8 Copy_u8Data)
{
    ...
    DIO_VidSetPinValue(LCD_CTRL_PORT_RS, LCD_RS_PIN, PinHigh); // RS read Data
    DIO_VidSetPinValue(LCD_CTRL_PORT_RW, LCD_RW_PIN, PinLow);  // RW write to LCD
...
void LCD_voidInit(void)
{
    ...
    DIO_VidSetPinDirection(LCD_CTRL_PORT_RS, LCD_RS_PIN, Output);
    DIO_VidSetPinDirection(LCD_CTRL_PORT_RW, LCD_RW_PIN, Output);
    DIO_VidSetPinDirection(LCD_CTRL_PORT_E, LCD_E_PIN, Output);
...
void LCD_voidGoToRowColumn(unsigned char Row, unsigned char Column)
{
    ...
    unsigned int addr = (64 * Row) + Column;

    // Send command to set DDRAM address
    LCD_voidSendCommand(128 + addr);
...
{
    uint8 CGRAM_address = 0, Iterator = 0;
    ...
    when we send a character to lCD like 'A', the micro controller of the LCD looks the ASCII code
